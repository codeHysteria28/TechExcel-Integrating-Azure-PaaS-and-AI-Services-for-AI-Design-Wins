...
def get_hotels():
    ...
    response = requests.get(f"https://{api_endpoint}/Hotels", timeout=10)
    return response
...
def get_hotel_bookings(hotel_id):
    ...
    api_endpoint = st.secrets["api"]["endpoint"]
    response = requests.get(f"https://{api_endpoint}/Hotels/{hotel_id}/Bookings", timeout=10)
    return response
...
def invoke_chat_endpoint(question):
    ...
    api_endpoint = st.secrets["api"]["endpoint"]
    response = requests.post(f"https://{api_endpoint}/Chat", data={"message": question}, timeout=10)
    return response
